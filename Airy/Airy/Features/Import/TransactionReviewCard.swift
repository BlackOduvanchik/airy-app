...
struct TransactionReviewCard: View {
        ...
        HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                Text(formatAmount)
                    ...
                    .foregroundColor(isIncome ? theme.incomeColor : theme.expenseColor)
            }
        }
    }
    ...
    private var typeBadge: some View {
        HStack(spacing: 3) {
            ...
            Text(isIncome ? L("common_income") : L("common_expense"))
                .font(.system(size: 10, weight: .bold))
                .textCase(.uppercase)
                .tracking(0.04)
        }
        ...
        .padding(.vertical, 2)
        .background(isIncome ? theme.incomeColor.opacity(0.15) : theme.expenseColor.opacity(0.1))
        .foregroundColor(isIncome ? theme.incomeColor : theme.expenseColor)
        .clipShape(RoundedRectangle(cornerRadius: 6))
    }
    ...
    private var pillRow: some View {
        HStack {
            categoryPill
            if matchedSubscriptionInterval != nil {
                subscriptionBadge
            }
            Spacer()
    ...
    private var categoryPill: some View {
        HStack(spacing: 6) {
            Image(systemName: categoryIcon)
                .font(.system(size: 14))
            if let sub = subcategoryLabel {
                Text("\(categoryLabel) › \(sub)")
    ...
    private var ruleRow: some View {
        HStack {
            Text(L("review_remember", merchant))
                .font(.system(size: 12))
                .foregroundColor(theme.textTertiary)
                .lineLimit(1)
    ...
    private var cardBorderColor: Color {
        isLowConfidence ? Color(red: 0.851, green: 0.627, blue: 0.357).opacity(0.3) : theme.glassBorder
    }
    ...
    private static func currencyFormatter(for code: String) -> NumberFormatter {
        let key = (code.isEmpty ? "USD" : code) as NSString
        if let cached = currencyFormatterCache.object(forKey: key) { return cached }
    ...
    private var formattedDate: String {
        ...
        guard let d = Self.inputDateFormatter.date(from: String(payloadDate.prefix(10))) else { return payloadDate }
        let cal = Calendar.current
        if cal.isDateInToday(d) {
            if let t = time, !t.isEmpty {
                return L("review_today", formatTime(t))
            }
            return L("common_today")
        } else if cal.isDateInYesterday(d) {
            if let t = time, !t.isEmpty {
                return L("review_yesterday", formatTime(t))
            }
    ...
    private var formatAmount: String {
        AppFormatters.formatTransaction(amount: amount, currency: currency, isIncome: isIncome)
    }
}
