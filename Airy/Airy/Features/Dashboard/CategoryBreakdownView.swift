...
struct CategoryBreakdownView: View {
        ...
        return GeometryReader { geo in
            ...
            ZStack {
                ...
                ArcSegmentShape(startAngle: .degrees(0), endAngle: .degrees(359.99))
                    .stroke(
                        Color.white.opacity(theme.isDark ? 0.05 : 0.2),
    ...
    private func selectedCategoryTooltip(segment: CategorySegment) -> some View {
        ...
        .padding(.vertical, 10)
        .background(Color.white.opacity(theme.isDark ? 0.12 : 1))
        .clipShape(RoundedRectangle(cornerRadius: 12))
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(theme.bgTop, lineWidth: 1)
        )
...
final class CategoryBreakdownViewModel {
    ...
    var monthLabel: String = {
        return AppFormatters.monthYear.string(from: Date())
    }()
    var monthKey: String = {
